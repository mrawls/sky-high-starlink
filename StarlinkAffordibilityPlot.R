...
countryLab$p=seq(0.02,min(1,0.69/26*length(subset(fil,Pop>popLim)$Pop)),length.out = length(subset(fil,Pop>popLim)$Pop))
...
ggplot(data=fil,aes(Cost,InternetPercentage))+
  ...
  geom_rect(aes(xmin=0.00316,xmax=0.0125,ymin=0,ymax=min(1,0.69/26*length(subset(fil,Pop>popLim)$Pop))+0.02),fill="white",col="black")+
  geom_text(aes(x=Cost*(1+Pop^0.5/110),label=ifelse(Pop>popLim,as.character(CountryCode),''),col=Category3),hjust=0,vjust=0.5,family="Segoe UI Semilight",size=5)+
  geom_text(data=countryLab,aes(x=0.0047,y=p,label=CountryName),hjust=0,family="Segoe UI Semilight",size=5)+
  geom_text(data=countryLab,aes(x=0.0033,y=p,label=CountryCode),hjust=0,family="Segoe UI Semilight",size=5)+
  ...
  theme_light()+
  theme(aspect.ratio = 0.6,text=element_text(size=22,family="Segoe UI Semilight"),plot.title = element_text(size=22,hjust = 0.5,face="bold"))
  
ggsave("affordability.png",path="D:/Users/victo/Desktop/Zoom",device="png",dpi=500) #save the plot with the desired name at the desired path

sum(subset(fil,Pop>popLim)$Pop)/sum(fil$Pop)

length(subset(fil,Pop>popLim)$Pop)
